...
(() => {
  ...
  function validateCheckout() {
    for (const id of ['customer', 'change']) {
  ...
  $('checkout-button').addEventListener('click', () => {
    ...
    togglePaymentFields(); $('checkout-dialog').showModal();
  });
  for (const id of ['payment', 'needs-change']) $(id).addEventListener('change', togglePaymentFields);
  $('checkout-form').addEventListener('submit', event => {
    event.preventDefault();
    const values = validateCheckout();
    if (!values) return;
    // Capture the complete order before clearing any state used by the message.
    const finalWhatsappUrl = `${whatsappUrl}?text=${encodeURIComponent(buildMessage(values))}`;
    cart = [];
    persistCart();
    $('checkout-form').reset();
    $('product-form').reset();
  ...
  restoreCart(); renderProducts(); renderCart(); togglePaymentFields();
})();
